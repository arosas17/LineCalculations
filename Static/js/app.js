    ...
    for(var colm = 0; colm <= 9; colm++){
      ...
      if (colm == 1){
        if (rawData[row][colm] != null){
          ...
          try {
            selectedDate = rawData[row][colm].toLocaleDateString('en-US');
...
//Sorting the Rows
  ...
  var sortedRows = rows.sort((a, b) => {
    ...
    if (column == 0 || column == 4 || column == 5 || column == 7 || column == 8 || column == 9){
...
// Making it so that blank spaces are placed at top
...
document.getElementById("changeVal").addEventListener('click', () => {
  ...
  else if (chosenColm == "SKU"){chosenColm = 4}
  else if (chosenColm == "Min"){chosenColm = 5}
  else if (chosenColm == "Target"){chosenColm = 6}
  else if (chosenColm == "Fill Weight"){chosenColm = 7}
  else if (chosenColm == "Torque"){chosenColm = 8}
  else if (chosenColm == "Overfill %"){chosenColm = 9}
